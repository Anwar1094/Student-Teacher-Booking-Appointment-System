...
function validate() {
    ...
    if (this.id == 'login' && uid.length >= 3 && pass.length >= 8) {
        ...
        readStu(() => {
            try {
                correctUser = true
                localStorage.setItem('Email', globalData[uid].email)
                ...
                if (correctUser == true && correctPass == true) {
                    swal("Login Success", `Welcome ${globalData[uid].username}! As Student`, 'success').then((result) => {
                        let id = globalData[uid].username
                        location.href = 'StuWin/Stuwin.html'
                        uid = id
